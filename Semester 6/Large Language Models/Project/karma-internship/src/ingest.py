...
class DocumentIngestor:
    ...
    def plot_embeddings(self, embeddings):
        ...
        Args:
        - embeddings (list): List of embeddings to visualize.
        Returns:
        - None
        """
        ...
        for i in enumerate(embeddings):
            plt.scatter(
                reduced_embeddings[i, 0], reduced_embeddings[i, 1], label=f"Question {i + 1}")
            plt.annotate(
                f"Question {i + 1}", (reduced_embeddings[i, 0], reduced_embeddings[i, 1]))
    ...
    def ingest(self):
        ...
        self.plot_embeddings(embeddings)
